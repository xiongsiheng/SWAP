...
parser.add_argument('--batch_size', type=int, default=1) # the batch size for training
parser.add_argument('--max_seq_len', type=int, default=2048) # the maximum sequence length for the model
...
for sample in dataset['train']:
    ...
    # remove the samples with more than group_size responses.
...
# print the example prompt for the discriminator
if f_print_example:
    for i in range(5):
        sample = dataset_train[i]
        prompt = generate_prompt_for_discriminator(args.prob_type, sample['input'], sample['output'], eos_token="<|eot_id|>", use_graph=args.use_graph,
...
if f_train:
    def formatting_func(sample):
        ...
        for x, y in zip(sample['input'], sample['output']):
            op = generate_prompt_for_discriminator(args.prob_type, x, y, eos_token="<|eot_id|>", use_graph=args.use_graph,
                                                   use_meta_knwoledge=args.use_meta_knowledge)
            output.append(op)
        return output
