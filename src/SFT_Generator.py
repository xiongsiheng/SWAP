...
def parse_args():
    ...
    parser.add_argument('--resume_from', default=None)  # the path to the resume checkpoint
...
def filter_dataset(dataset, args):
    ...
    dataset_filtered = [
        ...
        if 'subset' in sample and (args.subset == 'all' or args.subset == sample['subset'])
    ]

    split_idx = int(0.8 * len(dataset_filtered))
    dataset_train = Dataset.from_list(dataset_filtered[:split_idx])
    dataset_val = Dataset.from_list(dataset_filtered[split_idx:])
...
def main():
    ...
    if args.train:
        def formatting_func(sample):
            return [
                generate_prompt_for_generator(
                    args.prob_type, x, y, eos_token="<|eot_id|>", use_graph=args.use_graph
                )
        ...
        SFT_with_LoRA(
            ...
            dataset_train, dataset_val, args.batch_size, args.max_seq_len,
            args.resume_from, collator
        )
