...
parser.add_argument('--batch_size', type=int, default=6) # the batch size for training
parser.add_argument('--max_seq_len', type=int, default=512) # the maximum sequence length for the model
...
dataset_train = Dataset.from_dict(obtain_sem_equ_data(dataset_filtered[:int(0.8*len(dataset_filtered))]))
dataset_val = Dataset.from_dict(obtain_sem_equ_data(dataset_filtered[int(0.8*len(dataset_filtered)):]))
...
# print the example prompt for the semantic equivalent lora
if f_print_example:
    for i in range(5):
...
if f_train:
    def formatting_func(sample):
        ...
        '''
        output = []
        for I, x, y in zip(sample['instruction'], sample['input'], sample['output']):
            op = generate_prompt_for_sem_equ_lora_train(I, x, y, eos_token="<|eot_id|>")
            output.append(op)
        return output
